...
> **Note: This file is actually named `.env`, not `azuracast.env`, which is a separate file.** Most Linux hosts will hide the `.env` file by default, but you can still see it by running `ls -lah` or edit it with a command like `vim .env`.
{.is-info}
